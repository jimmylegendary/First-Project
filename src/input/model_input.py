...
class ModelInput:
    ...
    def select_mortality_ahi_subject(self, total_data: pd.DataFrame):
        ...
        alive: pd.DataFrame = df_ahi[
            df_ahi["vital"] == 1 | ((df_ahi["vital"] == 0) & (df_ahi["censdate"] > 365 * 15))
        ].reset_index(drop=True)
        alive["vital"] = 1
        deceased: pd.DataFrame = df_ahi[
            (df_ahi["vital"] == 0) & (df_ahi["censdate"] <= 365 * 15)
        ].reset_index(drop=True)
        deceased["vital"] = 0
    ...
    def select_cvd_ahi_subject(self, total_data: pd.DataFrame):
        ...
        any_cvd: pd.DataFrame = df_pre_cvd[
            df_pre_cvd["any_cvd"] != 0 & df_pre_cvd["cvd_date"] != 0
        ].reset_index(drop=True)
        any_cvd[:, "any_cvd"] = 1
    ...
    def make_dataset(self, data_path):
        ...
        self.dataset = MyDataset(feature_name=feature_name, x=x, y=y)
